// MD5 implementation for Last.fm API signature generation
// Based on the algorithm by Paul Johnston / blueimp (MIT license)
...
function binl2rstr(input) {
    let out = '';
    for (let i = 0, len = input.length * 32; i < len; i += 8)
        out += String.fromCharCode((input[i >> 5] >>> (i % 32)) & 0xff);
    return out;
}

function rstr2binl(input) {
    const out = new Array((input.length >> 2) + 1).fill(0);
...
function rstr2hex(input) {
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        out += hex[(x >>> 4) & 0x0f] + hex[x & 0x0f];
    }
    return out;
}

export default function md5(input) {
    const rstr = unescape(encodeURIComponent(input));
    return rstr2hex(binl2rstr(binlMD5(rstr2binl(rstr), rstr.length * 8)));
}
