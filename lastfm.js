...
export class LastFMScrobbler {
    constructor() {
        this.DEFAULT_API_KEY = '65903c2e5704e2eca76cf3faa66b4786';
        this.DEFAULT_API_SECRET = 'e47b2a9a4baf71a3cb9e77639a769821';
    ...
    saveSession(sessionKey, username) {
        ...
        localStorage.setItem('lastfm-session', JSON.stringify({ key: sessionKey, name: username }));
    }
    ...
    async makeRequest(method, params = {}, requiresAuth = false) {
        const requestParams = { method, api_key: this.API_KEY, ...params };
    ...
    async authenticateWithCredentials(username, password) {
        ...
        const data = await response.json();
        if (data.error) throw new Error(data.message || 'Last.fm authentication error');
        if (data.session) {
            this.saveSession(data.session.key, data.session.name);
            return { success: true, username: data.session.name };
        }
    ...
    async updateNowPlaying(track) {
        ...
        try {
            ...
            if (track.trackNumber) params.trackNumber = track.trackNumber;
    ...
    async scrobbleCurrentTrack() {
        ...
        try {
            ...
            if (this.currentTrack.trackNumber) params.trackNumber = this.currentTrack.trackNumber;

            await this.makeRequest('track.scrobble', params, true);
            this.hasScrobbled = true;
            console.log('Scrobbled:', scrobbleTitle);
        } catch (error) {
    ...
    async loveTrack(track) {
        if (!this.isAuthenticated()) return;
        try {
            await this.makeRequest('track.love', {
                artist: this._getScrobbleArtist(track),
                track: track.title,
            }, true);
